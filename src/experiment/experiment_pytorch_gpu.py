...
class PyTorchGPUExperiment(Experiment):
    tool_name = "pytorch_gpu"
    ...
    def train(self, ctx):
        ...
        x_train = torch.from_numpy(ctx.x_scaled).float().to(device)
        y_train = torch.from_numpy(ctx.y_scaled).float().to(device)
    ...
    def simulate(self, ctx, x):
        ...
        return ctx.model.forward(x_torch).detach().to("cpu").numpy()
