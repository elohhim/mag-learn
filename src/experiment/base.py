...
class Experiment(ABC):
    ...
    def run(self):
        ...
        for dataset in datasets:
            ...
            ctx = ExperimentContext(dataset, x_scaler, y_scaler, x_scaled,
                                    y_scaled, model, 1e-4)
            log_time = dict()
            history = self._train(ctx, log_time=log_time, log_name="time")
            self.plot_results(ctx, pathlib.Path(output_dir, file_prefix),
                              history, log_time["time"])

    @timeit
    def _train(self, ctx, **kwargs):
    ...
    def generate_datasets(self, noise=False):
        ...
        fun_definitions.extend(
            ...
            for n in [100, 10_000, 1_000_000])
        datasets = []
        for fun_name, fun, samples, *ranges in fun_definitions[7:8]:
    ...
    def plot_results(self, ctx: ExperimentContext, output_path, history, time):
        ...
        self.plot_approximation(ctx, projection, output_path, time)
        self.plot_absolute_error(ctx, projection, output_path)
        self.plot_relative_error(ctx, projection, output_path)
        if history is not None:
            self.plot_history(history, output_path, time)

    def plot_function(self, ctx, projection, output_path):
        fig = plt.figure()
        plt.title(f"Function: {ctx.dataset.fun_name}")
        ax = plt.subplot(1, 1, 1, projection=projection)
        plot_ranges = [(lo, hi, 100) for lo, hi in ctx.dataset.ranges]
        plot_fun(ctx.dataset.fun, *plot_ranges, ax=ax)
        fig.savefig(f'{output_path}_01_function.png', dpi=600)
        plt.close(fig)

    def plot_approximation(self, ctx, projection, output_path, time):
        fig = plt.figure()
        plt.title(f"Approximation of {ctx.dataset.fun_name}\nby NN trained on "
                  f"{ctx.dataset.samples} samples. Time {time}ms.")
        ax = plt.subplot(1, 1, 1, projection=projection)
        plot_ranges = [(lo, hi, 100) for lo, hi in ctx.dataset.ranges]
        plot_fun(lambda x: self._simulate(ctx, x), *plot_ranges, ax=ax)
        fig.savefig(f'{output_path}_02_approximation.png', dpi=600)
        plt.close(fig)

    def plot_absolute_error(self, ctx, projection, output_path):
        fig = plt.figure()
        plt.title("Approximation absolute error")
        ax = plt.subplot(1, 1, 1, projection=projection)
        plot_ranges = [(lo, hi, 100) for lo, hi in ctx.dataset.ranges]
        ...
        fig.savefig(f'{output_path}_03_absolute_error.png', dpi=600)
        plt.close(fig)
    ...
    def plot_relative_error(self, ctx, projection, output_path):
        ...
        fig.savefig(f'{output_path}_04_relative_error.png', dpi=600)
        plt.close(fig)

    def plot_history(self, history, output_path, time):
        fig = plt.figure()
        plt.title(f"Training history. Epoches: {len(history)}"
                  f" Loss: {history[-1]:.2e}\nTime: {time}ms")
        plt.xlabel("Epoch")
        plt.ylabel("MSE")
        plt.semilogy(history)
        fig.savefig(f'{output_path}_05_training_history.png', dpi=600)
        plt.close(fig)
    ...
    def plot_aggregate(self, ctx, projection, output_path):
        ...
        plot_fun(lambda x: (ctx.dataset.fun(x) - self._simulate(ctx, x)),
                 *plot_ranges, ax=ax)
        fig.savefig(f'{output_path}_06_aggregate.png', dpi=600)
        plt.close(fig)
